...
### 금액 규모 (226개 실제 공고 분석 기준)
| 구간 | 건수 | 비율 | 대표 사례 |
|------|------|------|-----------|
| ~3천만 | 54 | 24% | 보건소/복지관 1대 |
| 3천만~5천만 | 84 | **37%** | 업무용 차량 1~2대 (가장 많음) |
| 5천만~1억 | 54 | 24% | 지자체/공기업 2~4대 |
| 1억~3억 | 23 | 10% | 도청/교육청 대규모 |
| 3억~ | 10 | 4% | 경기도청 13억, 중기은행 167억 |
| **중위값** | | | **약 3,768만원** |

### 계약방식 분포
| 방식 | 비율 | 특징 |
|------|------|------|
| 수의계약 | 64% | 소액수의견적+수의시담 (소규모) |
| 제한경쟁 | 23% | 지역제한+업종제한 |
| 일반경쟁 | 13% | 대형 건 |
...
| 하한율 | 비율 | 적용 기준 |
|--------|------|-----------|
| 88.0% | 67% | 소액수의/제한적최저가 기본 |
| 87.745% | 14% | 적격심사-관리규정 |
| 84.245% | 7% | 임대차 적격심사 (고시금액 미만) |
| 80.495% | 2% | 5억 이상 대형 |
...
- **차별점**: 차량 렌트 특화 + 첨부파일 AI 분석 + 자격 체크/적격심사 시뮬레이션
...
- **AI**: Laravel AI SDK (공고 해석, 자격 체크, 시뮬레이션 보조)
- **알림**: 카카오 알림톡, 텔레그램, 이메일
- **배포**: Laravel Cloud
...
### vehicle_prices (차량 시세 참조 테이블)
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | bigint | PK |
| manufacturer | varchar | 제조사 (현대/기아/제네시스/BMW 등) |
| model | varchar | 모델명 (그랜저/소나타/K5 등) |
| trim | varchar | 트림 (프리미엄/익스클루시브/캘리그래피 등) |
| fuel_type | enum | gasoline/diesel/hybrid/electric/lpg |
| model_year | int | 연식 (2026) |
| new_price | bigint | 신차 가격 (원) |
| registration_cost | bigint | 취등록세 예상 (원, 신차가격×7%) |
| resale_12m | bigint | 1년 후 중고 예상가 (원, null=데이터없음) |
| resale_24m | bigint | 2년 후 중고 예상가 |
| resale_36m | bigint | 3년 후 중고 예상가 |
| resale_48m | bigint | 4년 후 중고 예상가 |
| insurance_annual | bigint | 연간 보험료 예상 (사업용, 대인무한+대물3억+자차) |
| source | varchar | 데이터 출처 (현대공식/다나와/엔카 등) |
| updated_at | timestamp | 시세 갱신일 |

> **시세 갱신 주기**: 연 2회 (연식변경+중간점검)
> **활용**: 공고 분석 카드에서 "예상 투입 범위" 자동 계산
> - 차종 매칭 → vehicle_prices에서 트림 범위 조회
> - 대수 × (new_price ~ new_price+registration_cost) = 예상 투입 범위
> - 36개월 후 중고 매각 예상가도 함께 표시

### vehicle_price_seed (초기 시드 데이터 — 2026년 기준)

> 주요 관용차 차종 신차가/중고 예상가

| 제조사 | 모델 | 트림 | 신차가 | 36개월 중고 예상 |
|--------|------|------|--------|------------------|
| 현대 | 그랜저 2.5 | 프리미엄 | 3,798만 | 2,500만 |
| 현대 | 그랜저 2.5 | 익스클루시브 | 4,287만 | 2,800만 |
| 현대 | 그랜저 2.5 | 아너스 | 4,513만 | 2,950만 |
| 현대 | 그랜저 2.5 | 캘리그래피 | 4,710만 | 3,100만 |
| 현대 | 그랜저 3.5 | 프리미엄 | 4,042만 | 2,650만 |
| 현대 | 소나타 2.0 | 프리미엄 | 2,943만 | 1,850만 |
| 현대 | 소나타 HEV | 프리미엄 | 3,287만 | 2,200만 |
| 현대 | 카니발 3.5 | 노블레스 | 4,282만 | 3,000만 |
| 현대 | 카니발 3.5 | 시그니처 | 4,721만 | 3,300만 |
| 현대 | 스타리아 | 투어러 9인승 | 3,775만 | 2,500만 |
| 현대 | 팰리세이드 3.8 | 프레스티지 | 4,338만 | 3,100만 |
| 현대 | 아이오닉5 | 롱레인지 | 5,200만 | 2,800만 |
| 기아 | K5 2.0 | 프레스티지 | 2,940만 | 1,800만 |
| 기아 | K8 2.5 | 노블레스 | 3,950만 | 2,600만 |
| 기아 | 쏘렌토 2.2D | 프레스티지 | 3,865만 | 2,600만 |
| 기아 | 카니발 3.5 | 노블레스 | 4,282만 | 3,000만 |
| 기아 | 스포티지 1.6T | 프레스티지 | 3,140만 | 2,000만 |
| 기아 | EV6 | 롱레인지 | 5,390만 | 2,700만 |
| 기아 | EV9 | 에어 6인승 | 7,386만 | 3,800만 |
| 기아 | 레이 | 프레스티지 | 1,615만 | 1,100만 |

> ※ 중고 예상가는 3년/6만km 기준, 사업용(허 번호판) 감가 반영
> ※ 전기차는 보조금 정책 변동으로 중고가 불확실성 높음 (±500만)

### bid_results (낙찰 결과 — Phase 2)
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | bigint | PK |
| bid_id | FK | bids.id |
| result_status | enum | awarded/rebid/failed/cancelled |
| awarded_company | varchar | 낙찰업체명 |
| awarded_biz_no | varchar | 낙찰업체 사업자번호 |
| awarded_amount | bigint | 낙찰금액 (원) |
| award_rate | decimal | 낙찰율 (투찰금액/예정가격 %) |
| participant_count | int | 참여업체 수 |
| award_dt | datetime | 낙찰일시 |
| raw_data | json | API 원본 |

### bid_result_rankings (투찰 순위 — Phase 2)
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | bigint | PK |
| bid_result_id | FK | bid_results.id |
| rank | int | 개찰순위 |
| company_name | varchar | 업체명 |
| bid_amount | bigint | 투찰금액 (원) |
| bid_rate | decimal | 투찰율 (%) |
| is_winner | boolean | 최종낙찰 여부 |

> **활용 (Phase 2)**:
> - 과거 차량임차 공고의 실제 낙찰율 분포 → "적정 투찰가 가이드"
> - 지역/규모/차종별 평균 경쟁률 분석
> - "이 조건의 공고는 보통 XX%에 낙찰됩니다" 인사이트
...
// 낙찰정보서비스 (Phase 2, 별도 API 키 필요)
const G2B_RESULT_BASE_URL = 'https://apis.data.go.kr/1230000/as/ScsbidInfoService/';
const G2B_RESULT_ENDPOINTS = [
    'service' => 'getScsbidListSttusServc',     // 용역 개찰결과
    'goods'   => 'getScsbidListSttusThng',       // 물품 개찰결과
    'etc'     => 'getScsbidListSttusEtc',        // 기타 개찰결과
];
const G2B_RESULT_SERVICE_ID = '15129397';
```
